...
phssr.makeHTMLPage = function(tableHtml, reporterOptions){
  ...
  if (typeof (reporterOptions.docTitle) !== 'undefined' && _.isString(reporterOptions.docTitle) ){
    ...
    staticHTMLContentprefix += '<title>Protractor Html Screenshots reporter</title> \n';
  }
    ...
    var str =
      '<table style="clear: both; width: 100%;"> \n' +
        '<tr> \n' +
          ...
          '<td style="width: 10%; overflow: hidden;"><a target="_blank" href="'+ path.basename(data.screenShotFile).replace('.png', '.json') +'">STACK TRACE</a></td> \n' +
...
function processJson(jsonData, options){
  ...
  var jsonStr = '<ul style="clear: both; width: 100%; padding: 0px; margin: 0px; list-style-type: none;">';
  formattedJson = [];
  currentFormattedDataIndex = 0;
